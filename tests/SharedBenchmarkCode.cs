...
public class SharedBenchmarkCode : SharedTestBase
{
    ...
    public async ValueTask IterationSetupAsync()
    {
    ...
    public void BenchmarkMethod(string methodName, Action action)
    {
        BenchmarkMethod(methodName, null, action);
    }

    public void BenchmarkMethod(string methodName, Action? setupAction, Action action)
    {
        ...
        for (var i = 0; i < Iterations; i++)
        {
            ...
            {
                setupAction?.Invoke(); // NOT timed

                var start = Stopwatch.GetTimestamp();
                action();         // ONLY THIS is timed
                var elapsed = Stopwatch.GetTimestamp() - start;
    ...
    public async ValueTask BenchmarkMethodAsync(string methodName, Func<Task> action)
    {
        await BenchmarkMethodAsync(methodName, null, action);
    }

    public async ValueTask BenchmarkMethodAsync(string methodName, Func<Task>? setupAction, Func<Task> action)
    {
        var testStartTime = Stopwatch.GetTimestamp();
        var log = new List<double>();
